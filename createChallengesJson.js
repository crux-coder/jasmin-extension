...
(async function () {
  Promise.all(responsiveWebDesignChallenges.map(async (challenge) => {
    return getVisual(challenge);
  })).then((res) => {
    fs.writeFileSync('test.json', JSON.stringify([...JSON.parse(fs.readFileSync('test.json')), ...res]));
...
//   'challenges.json',
//   JSON.stringify(
//     responsiveWebDesignChallenges.map((challenge) => {
//       request(challenge.url, (error, response, html) => {
...
//       })
//       return {
//         section: sectionName(challenge).trim(),
//         name: nameOfChallege(challenge).trim(),
//         url: challenge,
//         remove: [CONSTANTS.ALL_HELPERS],
//       };
//     })
//   )
// );
